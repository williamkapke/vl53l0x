...
export default class VL53L0X extends I2CCore {
  ...
  private async _getRangeMillimeters(continuous = false): Promise<number> {
    ...
    // while ((await this.readReg(REG.SYSRANGE_START)[0]) & REG.SYSTEM_SEQUENCE_CONFIG) {
    ...
    const range = await this._readReg(REG.RESULT_RANGE_STATUS + 10, true) // 0x1E
    await this._writeReg(REG.SYSTEM_INTERRUPT_CLEAR, REG.SYSTEM_SEQUENCE_CONFIG)
